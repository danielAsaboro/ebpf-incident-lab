...
export async function GET() { return Response.json({ configured: Boolean(process.env.INCIDENT_TUTOR_API_KEY && process.env.INCIDENT_TUTOR_MODEL), provider: "OpenAI", evidenceOptional: true }, { headers: { "cache-control": "no-store" } }); }
export async function POST(request: Request) {
  ...
  const key = process.env.INCIDENT_TUTOR_API_KEY;
  const model = process.env.INCIDENT_TUTOR_MODEL;
  ...
  try {
    ...
    const feedback = (result.output ?? []).filter((o: { type?: string })=>o.type === "message").flatMap((o: {content?: {type?:string;text?:string}[]})=>o.content??[]).filter((c:{type?:string})=>c.type === "output_text").map((c:{text?:string})=>c.text??"").join("\n");
    if (result.status !== "completed" || !feedback.trim()) return reply("The tutor response was incomplete. No assessment was recorded.", 502);
    return Response.json({feedback,provider:"OpenAI",assessment:"formative-only"},{headers:{"cache-control":"no-store"}});
