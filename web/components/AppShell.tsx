...
export function AppHeader() {
  ...
  return <header className="lab-header"><Link className="brand" href="/" aria-label="Incident Lab home"><span className="brand-symbol"><LabIcon name="layers" size={24}/></span><span>incident<span className="brand-light"> / lab</span><small>LINUX, UNDER THE SURFACE.</small></span></Link><nav className="global-nav" aria-label="Application navigation">{[{ href: "/", label: "Investigations" }, { href: "/guide", label: "Field guide" }, { href: "/notebooks", label: "My notebooks" }].map(item => <Link key={item.href} href={item.href} aria-current={(item.href === "/" ? pathname === "/" || pathname.startsWith("/labs/") : pathname === item.href) ? "page" : undefined}>{item.label}</Link>)}</nav><a className="source-link" href={repo} target="_blank" rel="noreferrer" aria-label="View source on GitHub"><LabIcon name="external" size={18}/></a></header>;
}
export function IncidentRail({ selected }: { selected?: string }) {
  const notes = useNotebooks();
  const completed = Object.values(notes).filter(note => note.phase === "complete").length;
  return <aside className="incident-rail" aria-label="Investigations"><Link href="/" className="rail-heading"><span className="micro">ALL INVESTIGATIONS</span><span className="count-badge">07</span></Link><nav className="incident-list" aria-label="Choose a lab">{labs.map((lab, index) => <Link key={lab.id} href={`/labs/${lab.id}`} className={`incident-item ${selected === lab.id ? "selected" : ""}`} aria-label={`${lab.id} ${lab.shortTitle}, ${lab.hosted ? "hosted lab" : "local VM"}`} aria-current={selected === lab.id ? "page" : undefined}><span className="incident-number">{lab.id}</span><span className="incident-item-content"><span className="incident-item-title">{lab.shortTitle}</span><span className="incident-item-status">{notes[lab.id]?.phase === "complete" ? "✓ NOTEBOOK COMPLETE" : notes[lab.id] ? "DRAFT SAVED" : lab.hosted ? "HOSTED LAB" : "LOCAL VM"}</span></span><LabIcon name={labIcons[index]} size={17}/></Link>)}</nav><div className="rail-progress"><span className="micro">YOUR FIELDWORK</span><strong>{completed}<span> / 7 notebooks</span></strong><div className="progress-track"><i style={{ width: `${completed / 7 * 100}%` }}/></div><Link href="/notebooks">Open your notes ↗</Link></div><div className="maker"><span className="maker-avatar">da.</span><span>Built by Daniel Asaboro<small>AN INDEPENDENT EXPLORATION</small></span></div></aside>;
}
export function AppFooter() { return <footer className="instrument-footer"><span><i className="status-dot"/>RUST + AYA<span className="footer-divider">/</span>eBPF INCIDENT LAB</span><span>MODELS EXPLAIN. EVIDENCE ESTABLISHES.</span><Link href="/guide#boundaries">Know the boundaries ↗</Link></footer>; }
