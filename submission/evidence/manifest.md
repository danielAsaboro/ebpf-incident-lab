...
| A checksum-pinned public release exists | incomplete | Requires release URL and matching downloaded checksum |
| The custom domain is live | incomplete | Requires DNS, TLS, and signed-out browser acceptance |
| Runner requests accept only fixed operations and one active job | verified | Runner API tests and hosted smoke; two-client queue/isolation test remains pending |
